            ...
            <aside :class="collapsed?'menu-collapsed':'menu-expanded'">
                ...
                <el-menu :default-active="$route.path" class="el-menu-vertical-demo" @open="handleopen" @close="handleclose" @select="handleselect" unique-opened router>
                    ...
                    <template v-for="item in  $router.options.routes" v-if="!item.hidden">
                         <div :key="item" v-if="item.titleActive" style="color:#888888;height: 40px;line-height: 40px;text-indent: 23px;">{{item.titleActive}}</div>
                        <el-submenu v-if="item.children && item.children.length" :index="item.path" :key="item.path">
                        
                            <template slot="title"><i :class="item.icon" style="padding-right:10px;"></i><span>{{item.name}}</span></template>
                            ...
                            <template v-for="itemChild in item.children" v-if="!itemChild.hidden">
                                <el-submenu v-if="itemChild.children && itemChild.children.length" :index="itemChild.path" :key="itemChild.path">
                                    <template slot="title"><i :class="itemChild.icon" style="padding-right:5px;" ></i><span>{{itemChild.name}}</span></template>
                                ...
                                <el-menu-item v-else :index="itemChild.path" :key="itemChild.path"><i :class="itemChild.icon"  style="padding-right:5px;"></i><span slot="title">{{itemChild.name}}</span></el-menu-item>
                            </template>
                        </el-submenu>
