<template>
	<section class="myself-container">
			...
			>
				...
				<div v-for="item in uploadsize" :key="item">
					<div class="dialog_div_con">
						<div class="dialog_div_title" v-if="item == 1">
							上传FULL包
						</div>
						<div class="dialog_div_title" v-else>上传DIFF包</div>
						<div class="dialog_div_warning">
							<p v-if="item == 1">
								*必传，full包为包含完整系统资源的文件，一般带有full后缀，如1.0.0_full.zip
							</p>
							<p v-else>
								*diff包为包含增量系统资源的文件，一般带有diff后缀，如：1.0.0_1.0.1_diff.zip
							</p>
						</div>
						...
						<div
							class="dialog_div_uploadinfo"
							v-if="uploadList[item - 1].showType"
						>
							<div>
								<span>版本号：</span
								>{{ uploadList[item - 1].info.rom_version }}
							</div>
							<div>
								<span>包名：</span
								>{{ uploadList[item - 1].info.version_name }}
							</div>
							<div>
								<span>类型：</span
								>{{ uploadList[item - 1].info.push_mod }}
							</div>
							<div>
								<span>大小：</span
								>{{ uploadList[item - 1].info.rom_size }}
							</div>
							<div>
								<span>md5：</span
								>{{ uploadList[item - 1].info.md5 }}
							</div>
						</div>
					</div>
				</div>
				...
				<div slot="footer" class="dialog-footer" style="">
					<el-button @click="dialogFormState = false"
						>取 消</el-button
					>
		...
		<el-dialog
			:visible.sync="dialogSetState"
			class="my_dialog_release"
			:show-close="false"
			:close-on-click-modal="false"
		>
			<el-table :data="arr">
				...
				<el-table-column prop="name" label="发布优先级渠道">
					<template slot-scope="scope">
						<el-radio v-model="radio" :label="scope.row.name"
							>优先</el-radio
						>
					</template>
				</el-table-column>
		...
		<el-dialog
			:visible.sync="dialogReleaseState"
			class="my_dialog_set"
			:show-close="false"
			:close-on-click-modal="false"
		>
					...
					if (res.status != 0) {
						...
						for (var i = 0; i < nowarractive.length; i++) {
							...
							for (var j = 0; j < sizearr.length; j++) {
								nowarractive[i].rom_size[
									j
								] = this.common.formatByteActive(sizearr[j]);
							}
						}
						...
						if (xhr.status == 200) {
							...
							if (headers.status == 0) {
								...
								_this.uploadList[itemActive].info.rom_version =
									headers.version;
								_this.uploadList[itemActive].info.version_name =
									headers.name;
								_this.uploadList[itemActive].info.rom_url =
									headers.url;
								_this.uploadList[itemActive].info.rom_size =
									headers.size;
								_this.md5Version = headers.version;
								_this.uploadList[itemActive].info.md5 =
									headers.md5;
								_this.uploadList[itemActive].info.push_mod =
									_this.pushType;
								_this.uploadList[itemActive].info.hashid =
									headers.hashid;
								_this.uploadList[itemActive].info.rom_desc =
									_this.textareaText;
								_this.uploadList[
									itemActive
								].info.dev_type = parseInt(_this.valuestate1);
