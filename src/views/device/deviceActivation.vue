	...
	<section class="myself-container deviceAvtive">
      ...
      <el-row>
		...
		<div class="devide_table">
			...
			>
				<el-col :span="6">
					<el-button type="primary" @click="addDev"
						>新建设备</el-button
					>
					<el-button type="primary" @click="importDev"
						>批量导入设备</el-button
					>
				</el-col>
				<el-col
			...
			<el-row type="flex" class="row_active row_dao">
				<el-col :span="24">
					<el-table
						:data="tableData"
						border
			...
			<div class="addaccout addaccout_add">
				...
				>
					...
					<el-form-item label="CPU-ID:" prop="cpu_id">
						<el-input
							v-model="ruleForm2.cpu_id"
							placeholder="请输入CPU-ID"
							style="width: 250px"
						></el-input>
					</el-form-item>
		...
		>
			<el-form ref="form">
				<el-form-item
					label="绑定ID:"
					style="dispaly: flex; justify-content: center"
				>
					<el-input
...
export default {
	data() {
		...
		let nameRule1 = (rule, value, callback) => {
			let regExp = new RegExp(
				/^SME[0-9a-zA-Z]{1}[0-9]{4}[0-9a-zA-Z]{7}$/
			);
	...
	methods: {
		...
		resetInfo() {
			...
			(this.import_time_value = ''), (this.time_value = '');
			this.searchText = '';
			this.is_activated = '';
			this.getInfo();
		},
		...
		getInfo() {
			var data = {
				...
				login_token: '',
				is_activated:
					this.is_activated === '' ? -1 : Number(this.is_activated),
				import_start_ts:
						...
						for (var i = 0; i < tempArr.length; i++) {
							...
							switch (tempArr[i].activate_ts) {
								...
								default:
									tempArr[
										i
									].activate_ts = this.common.getTimes(
			...
			device_cnt_overview(param)
				.then((res) => {
					if (res.status === 0) {
						...
						this.today_import_dev_cnt =
							res.data.today_import_dev_cnt;
						this.today_activated_dev_cnt =
							res.data.today_activated_dev_cnt;
					}
				})
				.catch((error) => {});
		},
		editBasicinfo() {
			if (this.ruleForm1.total_cap.toString().split('.')[1]) {
				if (
					this.ruleForm1.total_cap.toString().split('.')[1].length > 2
				) {
					this.$message({
						message: '总容量最多两位小数',
						type: 'error',
			...
			param.info = {
				...
				total_cap:
					parseInt(this.ruleForm1.total_cap) * 1024 * 1024 * 1024,
			...
			this.$refs[formName].validate((valid) => {
				if (valid) {
					...
					if (this.ruleForm2.total_capss.toString().split('.')[1]) {
						if (
							this.ruleForm2.total_capss.toString().split('.')[1]
								.length > 2
						) {
							this.$message({
								message: '总容量最多两位小数',
					...
					this.ruleForm2.total_cap =
						parseInt(this.ruleForm2.total_capss) *
						1024 *
						1024 *
						1024;
