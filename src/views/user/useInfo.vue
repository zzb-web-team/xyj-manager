...
export default {
	...
	methods: {
		...
		queryInfo() {
			...
			ptfs_query_list_user_store_list(params)
				.then((res) => {
					if (res.status == 0) {
						if (res.data.store_list.length > 0) {
							// this.dev_num = res.data.store_list[0].dev_num
							this.average_store =
								res.data.store_list[0].average_store / 100;
			...
			ptfs_query_user_profit_list(param)
				.then((res) => {
					...
					if (res.data.profit_detail_list) {
						...
						// 	nowarr[i].total_profit =
						// 		nowarr[i].total_profit / 100;
