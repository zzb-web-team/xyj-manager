			...
			>
						...
						>
							<tr>
								...
								<td class="device_title_content">
									{{ details_table.dev_sn }}
								</td>
								<td class="device_title_name">
									当前上/下行带宽
								</td>
								<td class="device_title_content">
                ...
                <td class="device_title_content">{{ details_table.mb_tem }}</td> -->
							...
							<tr>
								...
								<td class="device_title_content">
									{{
										details_table.hd_tmp != '-1℃'
											? details_table.hd_tmp
											: 'Unknown'
									}}
								</td>
							</tr>
					...
					<div class="monitorDetails-bottom">
						<div class="searach-con">
							<el-form ref="form" :model="form">
								<el-row
									type="flex"
									class="row_active row_active_monitor"
								>
									<el-form-item
										label="问题筛选"
										style="display: flex"
			...
			query_node_info(param)
				.then((res) => {
					if (res.status == 0 && res.err_code != 0) {
						...
						this.details_table.online_status =
							val.row.online_status;
						this.details_table.up_bandwidth = val.row.up_bandwidth;
						this.details_table.down_bandwidth =
							val.row.down_bandwidth;
						this.details_table.online_time = val.row.online_time;
						this.details_table.total_cap = val.row.total_cap;
						this.details_table.mem_ratio =
							res.data.mem_ratio / 100 + '%';
						// this.details_table.total_cap = res.data.total_cap / 100 + "%";
						this.details_table.cpu_tem =
							res.data.cpu_tem / 100 + '℃';
					...
					if (res.status == 0 && res.err_code != 0) {
						...
						if (res.data.dev_detail_info_list) {
							...
							for (var i = 0; i < nowarractive.length; i++) {
								nowarractive[
									i
								].time_stamp = this.common.getTimes(
								...
								nowarractive[
									i
								].user_cap = this.common.formatByteActive(
									nowarractive[i].user_cap
								);
								nowarractive[i].cpu_tem =
									nowarractive[i].cpu_tem / 100;
								nowarractive[i].mb_tem =
									nowarractive[i].mb_tem / 100;
								nowarractive[i].hd_tem =
									nowarractive[i].hd_tem / 100;
								nowarractive[i].mem_ratio =
									nowarractive[i].mem_ratio / 100;
								nowarractive[i].cap_ratio =
									nowarractive[i].cap_ratio / 100;
							}
					...
					if (res.status == 0 && res.err_code != 0) {
						...
						if (res.data.dev_detail_info_list) {
							...
							for (var i = 0; i < nowarractive.length; i++) {
								nowarractive[
									i
								].time_stamp = this.common.getTimeDay(
									parseInt(nowarractive[i].time_stamp * 1000)
								);
								...
								nowarractive[i].alarm_status =
									nowarractive[i].alarm_status;
								nowarractive[i].online_time =
									(
										nowarractive[i].online_time / 3600
									).toFixed(2) + 'h';
								nowarractive[i].total_cap =
									this.common.formatByteActive(
										nowarractive[i].use_cap
									) +
									'/' +
									this.common.formatByteActive(
										nowarractive[i].total_cap
