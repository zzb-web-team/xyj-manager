...
<el-table :data="tableData" style="width: 100%" @selection-change="handleSelectionChange" border >
    ...
    <el-table-column :label="tableOption.label" :width="tableOption.width" v-if="operatingStatus">
        <template slot-scope="scope">
            <a style="color:#6699ff;cursor: pointer;" size="mini" v-for="(item,index) in tableOption.options" :key="index" :type="item.type" @click="handleButton(scope.row,scope.$index)">
                {{item.label}}
            </a>
        </template>
    </el-table-column>
