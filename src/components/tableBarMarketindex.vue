...
<el-table :data="tableData" style="width: 100%" @selection-change="handleSelectionChange" border >
    <el-table-column type="selection" width="55" v-if="clomnSelection"> </el-table-column>
        <el-table-column label="LOGO">
         <template slot-scope="scope">
                        <span  style="cursor: pointer;"><img width="50" height="50" :src='scope.row.app_icon' /></span>
                    </template>
    </el-table-column>
    ...
    <el-table-column :label="tableOption.label" :width="tableOption.width" v-if="operatingStatus">
        <template slot-scope="scope">
            ...
            <span style="color:#6699ff;cursor: pointer;" @click="toOperating(scope.row)" type="text">{{ scope.row.state=='0'?'发布':'下架'}}</span>       
              <span style="color:#6699ff;cursor: pointer;" @click="toDetails(scope.row)" type="text">详情</span>
                <span style="color:#6699ff;cursor: pointer;" @click="toEdit(scope.row)" size="mini" type="text" >编辑</span>
</div>
        </template>
    </el-table-column>
</el-table>
