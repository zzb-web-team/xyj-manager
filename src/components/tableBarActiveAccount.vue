	...
	>
		...
		<el-table-column :label="tableOption.label" :width="tableOption.width">
			<template slot-scope="scope">
				<el-button @click="toDetail(scope.row)" size="mini"
					>详情</el-button
				>
				<el-button @click="toChange(scope.row)" size="mini"
					>修改</el-button
				>
				<el-button @click="changePassword(scope.row)" size="mini"
					>重置密码</el-button
				>
				<el-button
					@click="disable(scope.row)"
					:disabled="scope.row.id == uid || scope.row.id == 6"
					size="mini"
					>{{
